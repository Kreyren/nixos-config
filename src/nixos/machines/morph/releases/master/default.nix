...
in {
	...
	perSystem = { system, pkgs, inputs', self', ... }: {
		packages.nixos-morph-master-install = pkgs.writeShellApplication {
				...
				runtimeEnv = {
					systemDevice = self.nixosConfigurations.nixos-morph-master.config.disko.devices.disk.system.device;

					systemDeviceBlock = self.nixosConfigurations.nixos-morph-master.config.disko.devices.disk.system.device;

					secretPasswordPath = self.nixosConfigurations.nixos-morph-master.config.age.secrets.morph-disks-password.file;

					secretSSHHostKeyPath = self.nixosConfigurations.nixos-morph-master.config.age.secrets.morph-ssh-ed25519-private.file;
				};
